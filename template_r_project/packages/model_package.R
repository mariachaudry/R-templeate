...
setwd("./template_r_project/packages")

library(devtools)
library(roxygen2)
library(usethis)

usethis::create_package("model")
#go back to folder we started
setwd("..")
file.copy(from="./src/model/modelWithDoc.R", to="./packages/model/R/",overwrite = TRUE,copy.mode = TRUE)
updatePkgVer('./packages/model/')
document()
